...
@interface BySlideView ()<UITableViewDelegate,UITableViewDataSource,UIScrollViewDelegate>
...
- (instancetype)initWithFrame:(CGRect)frame withTitleArray:(NSArray *)titleArray withTopNumber:(NSInteger)topNum withTitleHeight:(CGFloat)titleHeight withURLArray:(NSArray *)URLArray withDelegate:(id<BySlideViewDelegate>)delegate{
    ...
    if (self) {
        ...
        CGFloat labelHeight = titleHeight/6.f;
        self.topLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, titleHeight - labelHeight, BtnWidth, labelHeight)];
        self.topLabel.backgroundColor = [UIColor redColor];
        [self.topScrollView addSubview:self.topLabel];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if (scrollView.tag - scrollTag == 1) {
        ...
        CGPoint nowPoint = scrollView.contentOffset;
        CGFloat nowX = nowPoint.x;
        CGFloat scaleX = nowX/scrollView.frame.size.width;
        
        CGFloat topNewX = self.topLabel.frame.size.width * scaleX;
        self.topLabel.frame = CGRectMake(topNewX, self.topLabel.frame.origin.y, self.topLabel.frame.size.width, self.topLabel.frame.size.height);
